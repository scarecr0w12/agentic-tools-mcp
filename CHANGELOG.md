...
### 🎯 Dashboard Integration - Interactive Task Management

This release introduces full dashboard integration with the MCP server, providing a beautiful web-based Kanban board for visual task management alongside the powerful MCP tools.

### Added

#### 📊 Dashboard Integration

- **CLI Flags**: New `--dashboard` flag to auto-launch dashboard with MCP server
- **Port Configuration**: New `--dashboard-port` flag to customize backend port (default: 4800)
- **Dashboard Launcher**: `src/utils/dashboard-launcher.ts` manages process spawning and cleanup
- **Automatic Startup**: Single command launches both backend (port 4800) and frontend (port 5173)
- **Process Management**: Proper cleanup and graceful shutdown of dashboard processes

#### 🎯 Interactive Kanban Board

- **4-Column Workflow**: To Do, In Progress, Blocked, Done
- **Drag & Drop**: Move tasks between columns to update status instantly
- **Visual Task Cards**: Display priority, complexity, tags, and time tracking
- **Priority Indicators**: Color-coded badges (P1-P10) for quick prioritization
- **Complexity Display**: Visual indicators for task complexity
...
- **Task Creation Modal**: Rich form with all metadata fields
  - Name and description
  - Priority (1-10 scale)
  - Complexity (1-10 scale)
  - Tags (comma-separated)
  - Estimated hours
- **Task Editing**: Update tasks via drag & drop or API
- **Task Deletion**: One-click delete with confirmation
- **Project Switching**: Dropdown selector to switch between projects

#### 🏗️ Backend API Routes

- **New Routes File**: `dashboard/backend/src/routes/projects.ts`
- **MCP Tool Proxy**: REST API proxies MCP tool calls via child process stdio
- **Project Endpoints**:
...
- **Keyboard Support**: Navigation and actions via keyboard

#### 📚 Documentation

- **Dashboard User Guide**: New `docs/DASHBOARD_GUIDE.md` with comprehensive usage instructions
- **Developer Documentation**: Updated `dashboard/README.md` with architecture and API reference
- **Architecture Updates**: `docs/dashboard/ARCHITECTURE.md` reflects implementation status
- **AI Coding Agent Instructions**: Added `.github/copilot-instructions.md` with comprehensive guidance

### Changed

- **Storage Config**: Extended `StorageConfig` interface with `launchDashboard` and `dashboardPort` fields
- **Server Initialization**: `src/index.ts` now integrates dashboard launcher on startup
- **Default Config**: `src/server.ts` updated with dashboard configuration defaults

### Technical Details

**New Dependencies**:

- Dashboard frontend: `@hello-pangea/dnd` for drag & drop functionality
- Dashboard backend: Enhanced Fastify routes for project/task management
...
No breaking changes. The `--dashboard` flag is optional and backward compatible. All existing MCP tools and functionality remain unchanged.
...
- Initial release with agent memories and task management
- Project-specific storage with working directory parameter
- Basic semantic search using hash-based embeddings
